...
public class DBManager {
    ...
    private static final String INSERT_TABLESPEEDTEST = "insert into " + DBHelper.tableSpeedTest + " (status) values (?);";
    ...
    private static final String INSERT_BEACONINFOLIST = "insert into " + DBHelper.tableBeaconInfoList + " (uuidFK,mac,beaconRssi," +
            "bTxPower,distance) values (?,?,?,?,?);";
    private static final String SELECT_ALL_BEACONINFOLIST = "select uuidFK,mac,beaconRssi,bTxPower,distance from " + DBHelper
            .tableBeaconInfoList;
    /* speed */
    ...
    private static final String SELECT_ALL_INDOORRECORD = "select signalStrength,cid,position," + "time,netType,networkType,lac,mnc,uuid," +
            "rsrp,rsrq,sinr,imsi from " + DBHelper.tableIndoorRecord;
    /* neighbor */
    private static final String DELETE_ALL_NEIGHBORLIST = "delete from " + DBHelper.tableNeighborRecord;
    private static final String INSERT_NEIGHBOR = "insert into " + DBHelper.tableNeighborRecord + " (uuidFK,cid,signalStrength,network," +
            "lac,time) values (?,?,?,?,?,?)";
    private static final String SELECT_NEIGHBOR = "select uuidFK,cid,signalStrength,network,lac,time from " + DBHelper.tableNeighborRecord;
    ...
    private static final String INSERT_BEACON = "insert into " + DBHelper.tableBeacon + " (_id,buildingName,buildingNumber,description," +
            "floor) values (?,?,?,?,?);";
    private static final String SELECT_BEACONS_IN_SAME_BUILDING = "select _id,floor,description,buildingName,buildingNumber from " +
            DBHelper.tableBeacon + " where buildingNumber = (select buildingNumber from " + DBHelper.tableBeacon + " where _id = ?) order" +
            " by floor";
    private static final String SELECT_BUILDING = "select buildingName,buildingNumber from " + DBHelper.tableBeacon + " where _id =? ";
    ...
    private static final String INSERT_INPECTION = "insert into " + DBHelper.tableInspection + "(username,startTime,endTime,duration," +
            "buildingId,province,city,longitude,latitude) " + "values(?,?,?,?,?,?,?,?,?)";
    private static final String SELECT_ALL_INSPECTION = "select username,startTime,endTime,duration," + "buildingId,province,city," +
            "longitude,latitude from " + DBHelper.tableInspection;
    /* inspector */
    private static final String DELETE_INPECTOR = "delete from " + DBHelper.tableInspector;
    private static final String INSERT_INPECTOR = "insert into " + DBHelper.tableInspector + "(username,phoneNumber,imsi,imei,province," +
            "city,companyName,password) " + "values(?,?,?,?,?,?,?,?)";
    private static final String SELECT_ALL_INSPECTOR = "select username,phoneNumber,imsi," + "imei,province,city,companyName,password" +
            " from " + DBHelper.tableInspector;
    ...
    private static final String INSERT_USER_SETTING = "insert into " + DBHelper.tableSetting + "(username,isTestSpeed) " + "values(?,?)";
    private static final String SELECT_USER_SETTING = "select username,isTestSpeed " + " from " + DBHelper.tableSetting + " where " +
            "username=?";

    /* beaconDebug */
    private static final String INSERT_BEACON_DEBUG = "insert into " + DBHelper.tableBeaconDebug + " (mac,time,beaconRssi,bTxPower," +
            "uuidFK) values(?,?,?,?,?)";
    private static final String DELETE_BEACON_DEBUG = "delete from " + DBHelper.tableBeaconDebug;
    private static final String SELECT_BEACON_DEBUG = "select mac,time,beaconRssi,bTxPower,uuidFK from " + DBHelper.tableBeaconDebug;

    /* tableTrainDataCollection */
    private static final String INSERT_TRAINDATA_DEBUG = "insert into " + DBHelper.tableTrainDataCollection + " (mac, beaconRssi,uuid," +
            "marker) values(?,?,?,?)";
    private static final String DELETE_TRAINDATA_DEBUG = "delete from " + DBHelper.tableTrainDataCollection;
    private static final String SELECT_TRAINDATA_DEBUG = "select mac, beaconRssi,uuid,marker from " + DBHelper.tableTrainDataCollection;
    ...
    public void insertBeaconInfo(BeaconInfo beaconinfolist) {
        db.execSQL(INSERT_BEACONINFOLIST, new Object[]{beaconinfolist.getUuidFK(), beaconinfolist.getMac(), beaconinfolist.getBeaconRssi
                (), beaconinfolist.getbTxPower(), beaconinfolist.getDistance()});
    }
    ...
    public void refreshLocalization(List<LocalizationBeacon> ids) {
        ...
        for (LocalizationBeacon bi : ids) {
            db.execSQL(INSERT_LOCALIZATION, new Object[]{bi.getMac(), bi.getBuildingName(), bi.getBuildingNumber(), bi.getDescription(),
                    bi.getX(), bi.getY(), bi.getFloor()});
        }
    ...
    public void refreshBeacon(List<InspectedBeacon> ids) {
        ...
        for (InspectedBeacon bi : ids) {
            db.execSQL(INSERT_BEACON, new Object[]{bi.getMac(), bi.getBuildingName(), bi.getBuildingNumber(), bi.getDescription(), bi
                    .getFloor()});
        }
        db.setTransactionSuccessful();
        db.endTransaction();
    ...
    //选出所有位置信息准备上传
    public List<CalculatePosition> selectAllCalPosition() {
        Cursor c = db.rawQuery(SELECT_ALL_CALPOSITION, new String[]{});
        List<CalculatePosition> list = new ArrayList<CalculatePosition>();
        while (c.moveToNext()) {
            int maxLikehoodX = c.getInt(c.getColumnIndex("maxLikehoodX"));
            int maxLikehoodY = c.getInt(c.getColumnIndex("maxLikehoodY"));
            int dealedMLX = c.getInt(c.getColumnIndex("dealedMLX"));
            int dealedMLY = c.getInt(c.getColumnIndex("dealedMLY"));
            int threeX = c.getInt(c.getColumnIndex("threeX"));
    ...
    public void insertIndoorRecord(IndoorRecord record) {
        db.execSQL(INSERT_INDOORRECORD, new Object[]{record.getSignalStrength(), record.getCid(), record.getPosition(), record.getTime(),
    ...
    public List<IndoorRecord> selectAllIndoorRecord() {
        ...
        while (c.moveToNext()) {
            ...
            list.add(new IndoorRecord(signalStrength, cid, position, time, netType, networkType, lac, mnc, uuid, rsrp, rsrq, sinr, imsi));
        }
        c.close();
        return list;
    }
    ...
    public void insertNeighborRecord(List<Neighbor> neighbors, String uuid) {
        ...
        for (Neighbor n : neighbors) {
            ...
            db.execSQL(INSERT_NEIGHBOR, new Object[]{uuid, n.getCid(), n.getSignalStrength(), n.getNetwork(), n.getLac(), n.getTime()});
        }
        db.setTransactionSuccessful();
        db.endTransaction();
    ...
    public void insertInspection(Inspection inspection) {
        db.execSQL(INSERT_INPECTION, new Object[]{inspection.getUsername(), inspection.getStartTime(), inspection.getEndTime(),
                inspection.getDuration(), inspection.getBuildingId(), inspection.getProvince(), inspection.getCity(), inspection
                .getLongitude(), inspection.getLatitude()});
    }
    ...
    public List<Inspection> selectAllInspection() {
        ...
        while (c.moveToNext()) {
            ...
            try {
                ...
                if (endTimeStr != null) endTime = new Timestamp(format.parse(endTimeStr).getTime());
            } catch (ParseException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }

            list.add(new Inspection(-1, username, startTime, endTime, duration, buildingId, province, city, longitude, latitude));
        }
        c.close();
        return list;
    }
    ...
    public boolean insertInspector(Inspector inspector) {
        ...
        db.beginTransaction();
        deleteInspector();
        db.execSQL(INSERT_INPECTOR, new Object[]{inspector.getUsername(), inspector.getPhoneNumber(), inspector.getImsi(), inspector
                .getImei(), inspector.getProvince(), inspector.getCity(), inspector.getCompanyName(), inspector.getPassword()});
        db.setTransactionSuccessful();
        db.endTransaction();
        return true;
    ...
    public void updateUserSetting(UserSetting us) {
        ...
        if (_us == null) {
            db.execSQL(INSERT_USER_SETTING, new Object[]{us.getUsername(), us.isTestSpeed() == true ? Integer.valueOf(1) : Integer
                    .valueOf(0)});
        } else {
            db.beginTransaction();
            db.execSQL(DELETE_USER_SETTING);
    ...
    public UserSetting selectUserSetting(String username) {
        ...
        while (c.moveToNext()) {
            ...
            if (itp > 0) isTestSpeed = true;
            else isTestSpeed = false;
            us = new UserSetting();
